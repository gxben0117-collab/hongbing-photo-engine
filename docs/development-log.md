...
## 2026-08-04　v4.39 全站 AI 判斷主題最佳姿勢

- **共用姿勢入口**：17 個具人物姿勢控制的正式頁新增／統一 `pose=auto`，介面顯示「AI 判斷｜主題最佳姿勢」；
  由共用 `HB_CORE_PROMPT.controls.autoPose` 依主題、服裝、構圖、鏡頭、光影與場景選擇單一合適姿勢。
- **選擇優先序**：具名姿勢與自訂姿勢仍優先於 auto；自訂姿勢不會再和預設姿勢疊加。旅拍、雜誌與公仔保留
  各自的主題／比例語氣；中式古典、和服、韓服與婚紗補上原本缺少的 auto UI 與資料映射。
- **邊界**：店家廣告沒有獨立人物姿勢控制；編輯視覺設計是 Stage 2 非破壞性平面編輯，必須保留來源照片原始姿勢，兩頁不加入 auto 姿勢欄位。
- **驗證**：新增 UI flow 的 auto pose 契約，並以靜態檢查、模板引用、Prompt 預覽、1900 組隨機稽核與桌面／手機瀏覽器回歸驗證。
...
- **共用範本**：以 `fantasy-fashion.html` 為正式人像頁的共同控制基準，統一 9 組拍攝角度、8 組
  圖片比例、5 組身形輪廓與 Layer 0／3／6／9／隨機 Layer；旅拍、雜誌與婚紗只保留必要的主題專屬追加。
- **構圖邊界**：一般幻想系頁面沿用幻想廣告的共同構圖語彙；中式古典、和服、韓服、婚紗、旅拍與
  雜誌保留各自文化／封面／旅拍敘事構圖，避免共同控制軸覆蓋真正的主題語言。
...
## 2026-08-03　v4.35 婚紗藝術寫真工具頁

- **獨立工具**：新增 bridal-editorial.html，定位為單人新娘的精品婚紗棚拍、婚紗廣告與婚紗雜誌寫真；不把雙人婚禮或傳統文化婚服混入本頁。
- **控制軸**：新增 12 組一鍵模板，完整涵蓋婚紗輪廓、工藝材質、服裝改造 Layer、頭紗、配件、身形、姿勢、妝髮、膚質、光影、配色、背景、鏡頭與比例。
- **身份保護**：新增 bridalEditorial page core，沿用全站身份／臉部幾何／真人骨架核心；頭紗身份保護只在選擇頭紗時條件式追加，覆面頭紗不進一般隨機池。
- **工程整合**：首頁、check-static、check-ui-flows、validate-preset-refs、audit-100x 與 build-prompt-preview 均納入第 19 個正式工具頁。
- **介面關聯回歸**：修正構圖選項的左右留白文案、水晶材質的介面命名，並把 Layer 0／3／6／9 的說明明確限定為「隨機套用時」的改造數量；手動部位選擇仍獨立保留。選項卡的原生 input 也覆蓋整張卡片，讓滑鼠、觸控與鍵盤選取一致。
- **驗證**：婚紗頁 12 組模板引用、UI flow、靜態檢查與完整 100 組隨機模擬均通過；全站稽核共 1900 組，0 issue。

## 2026-08-03　v4.34 服裝改造核心主題化擴充

- **主題化資料**：為 14 個正式服裝改造頁補上主題專屬的胸口、腰側與肩部細節；保留既有手動選項，
  不把所有頁面硬塞成仙俠的 71 項規模。和服、韓服、中式古典維持文化服裝輪廓，戰鬥學院維持制服與
  裝備功能，幻想、晚宴、花卉、偶像與動漫頁各自使用相容語彙。
- **共用行為**：新增 `assets/garment-core.js`，集中 Layer 解析、只補未手動選取部位、主題隨機池
  篩選與完整 Prompt 片段去重；14 頁都接入同一份 helper，沒有修改 `assets/core-prompt.js` 的身份、
  臉部幾何或真人骨架核心。
- **模板與隨機**：每頁至少 3 組代表性一鍵模板改用主題專屬細節；Layer 0／3／6／9／隨機 Layer
  維持原契約，傳統服飾與戰鬥制服的既有高風險剪裁仍只保留手動使用，不進主題隨機池。
- **回歸檢查**：`check-static.mjs`、`check-ui-flows.mjs`、`validate-preset-refs.mjs`、
  `audit-100x.mjs`、`build-prompt-preview.mjs` 與 `git diff --check` 全部通過；全站 18 頁共完成
  1,800 組隨機模擬，並新增 14 頁主題資料／UI／隨機池／代表模板契約檢查。

## 2026-08-03　v4.33 唯美古風寫真擴充

- **定位**：將中式古典頁的「新式古風」整理為「唯美古風」，明確區分歷史朝代服裝與中國古典元素的現代 Fine Art 寫真；不新增獨立頁，也不混入仙俠、花仙子或西式幻想語彙。
- **服裝與資料**：服裝由四組十五套擴充為四組十七套，新增古風宮廷華服、古風薄紗華服；補充宮廷步搖、荔枝果盤、御池端坐、宮苑黃金晨光、白玉御苑湯池與荔枝宮苑等資料。
- **一鍵模板**：`themeTemplates` 由 12 組擴充為 18 組，分成朝代古典、新式改良、唯美古風寫真三組；每組完整回填風格、構圖、服裝、材質、飾品、姿勢、光影、配色、背景、鏡頭與比例。
- **防回歸**：`check-ui-flows.mjs` 新增中式古典頁 18 組模板與三組分區契約；`audit-100x.mjs` 加強正向 Prompt 的仙俠、花仙子、女神、月宮與西式幻想語彙檢查。
- **身份邊界**：模板不寫入楊貴妃等人物姓名，也不重複加入臉部、膚色與品質宣稱，保留既有身份鎖定與真人骨架核心。

## 2026-08-03　介面文案關聯性回歸

- **檢查範圍**：盤點首頁與 18 個正式工具頁的頁面標題、區塊標題、提示文字、一鍵模板描述與
  首頁數量宣稱，確認文字和實際主題、控制項與資料池一致。
- **修正內容**：主題材質區改用各頁專屬命名；修正神話、奇幻、花卉、晚宴、異世界、偶像、仙俠
  等頁面複製殘留的服裝示例與坐姿情境；首頁 metadata、主標語與花漾甜美／晚宴名媛／韓系偶像／
  戰鬥學院的數量描述同步到實際內容。
- **防回歸**：`check-ui-flows.mjs` 新增主題文案契約與首頁 stale 文案檢查，不改動選項 key、
  Prompt 組裝或身份核心。
- **驗證**：18 頁靜態檢查、UI flow、preset 引用、1800 組隨機稽核、Prompt 預覽、diff 檢查，
  以及桌機／手機版面與 console 檢查全部通過。

## 2026-08-03　v4.32 編輯視覺設計 31 組一鍵模板

- **模板分類**：`editorial-identity.html` 新增並整理 31 組一鍵模板：時尚編輯 6、美業廣告 4、
  動漫電玩 4、電影海報 7、寫真書 5、旅遊設計 5。移除運動競技方向，不建立犯罪、恐怖或心理
  驚悚模板，也不提供分類隨機按鈕。
- **控制層**：新增文案語氣、主圖形之外的輔助標記、資訊欄 metadata 與 credit line；每個模板
  完整設定版型、人物版位、字體、文案、圖形、色彩、影像處理、印刷質感、留白、語言與比例，
  套用後仍保留使用者自行輸入的文字欄位。
- **生成邊界**：Stage 2 只在既有照片上做非破壞性編輯設計，明確鎖定人物、臉部、髮型、妝容、
  服裝、身形、姿勢、光線、鏡頭與背景；不得自行捏造產品、成分、醫療宣稱、目的地、座標、
  署名、日期或其他事實資訊。
- **驗證**：新增編輯頁模板契約與逐組稽核；UI flow、preset 引用、18 頁 × 100 組隨機稽核、
  固定 Prompt 預覽、Node 語法與靜態檢查均納入 31 組模板。

## 2026-08-03　v4.31 服裝改造核心 Layer 全站同步

- **同步範圍**：既有九頁 Layer 邏輯維持不變；為雜誌棚拍、幻想廣告、中式古典、日本和服、
  韓國韓服五頁補齊 Layer 0／3／6／9／隨機 Layer，使所有具胸口、腰側、肩部三區改造的正式頁
  都符合相同的「服裝改造核心」契約。
- **互動邏輯**：Layer 只決定隨機套用時要改造的部位數；先保留使用者手動選取的部位，再從未選
  部位補足目標數量。`random` 會在每次隨機套用時重新抽 Layer，Layer 本身不輸出到 Prompt。
- **模板一致性**：雜誌與幻想廣告模板重設為 Layer 0；中式古典、和服、韓服模板依胸口、腰側、
  肩部實際啟用數自動回填對應 Layer，避免 UI 狀態與模板內容不一致。
- **契約驗證**：`check-ui-flows.mjs` 新增服裝改造 Layer 契約，要求完整選項、對照常數、手動保留
  與未選部位補足邏輯同時存在，再執行全套靜態、preset、100 次隨機與 Prompt 預覽檢查。

## 2026-08-03　v4.30 三頁介面重整與真人骨架核心強化

- **介面重整**：中式古典、和服、韓服三頁將自訂服裝、材質／紋樣、姿勢、配色與背景欄位移回
  各自的 03、04A、07、09B、10 控制區；08 自訂要求只保留畫面強度與其他要求。
- **背景控制**：10 移除殘留的 10A 顯示標題，保留單一背景選擇，新增 `pureWhiteBackground` 與
  自訂背景欄位；既有生成、模板、隨機與資料驗證共用同一個 background key。
- **共用核心**：只修改 `assets/core-prompt.js` 的 `CORE_REALISTIC_ANATOMY`，補上頭、頸、鎖骨、
  胸廓與脊椎的連續骨架因果，以及禁止合成臉、浮動臉、獨立旋轉頭部等失敗模式；不新增平行身份核心。
- **驗證**：執行靜態檢查、UI flow、preset 引用、18 頁 × 100 組隨機稽核、固定 Prompt 預覽、
  Node 語法檢查與 `git diff --check`。
...
## 2026-08-03　中式古典美學服裝主題擴充

- **改動**：03 服裝主題由六套擴充為漢風、盛唐、宋韻、新式古風四組十五套；新增漢風直裾深衣、
  漢風曲裾繞襟、盛唐齊胸襦裙、盛唐翻領胡服、宋韻對襟褙子、宋韻短衫長裙，以及古風寫真華服、
  網紅古風華服、新式古風禮服。
- **生成邏輯**：每套服裝都有獨立英文 `garmentData` 描述；朝代款保留各自的服裝骨架，
  新式古風以現代古風商業造型為主，不引入仙俠或科技幻想詞彙。
- **一鍵與隨機**：12 組模板與隨機服裝池同步採用新增選項，模板涵蓋直裾、曲裾、齊胸、翻領、
  對襟褙子、短衫長裙及三組新式古風方向。
- **驗證**：重新執行 UI flow、preset 引用、16 頁 × 100 組隨機稽核、固定 Prompt 預覽、靜態檢查與
  `git diff --check`，共用身份與臉部幾何核心未修改。

## 2026-08-03　中式古典美學控制軸擴充

- **改動**：02 構圖補充衣袖、裙襬、門框、屏風與前景花枝展示構圖；07 姿勢補充整理衣襟、
  展示寬袖、捧茶盞、閱讀書卷與輕提裙襬等自然古典動作。
- **分層**：04 拆成材質／紋樣最多 2 個與中式飾品單選；09 拆成光影方式與色彩系統；
  10 拆成背景場景與留白方向。留白只保留後製空間，不生成文字。
- **組裝**：一鍵模板、隨機套用、固定 Prompt 預覽與 100 次稽核同步採用 accessory、
  colorPalette、whitespace 三個新欄位；共用身份與臉部幾何核心未修改。
- **修正**：團扇英文改為 round silk fan；梅蘭竹菊英文補上 chrysanthemum，避免 UI 與
  實際生成內容不一致。
- **驗證**：16 頁 UI flow、preset 引用、1600 組隨機模擬、固定 Prompt 預覽與靜態檢查通過。
...
- **改動**：仙俠、奇幻廣告、動漫角色、花仙子、異世界、花卉甜美、晚宴名媛、韓系偶像、
  古代女神統一採用「版型／構圖 → 服裝輪廓 → 主題材質 → 服裝變化 → 身形 → 姿勢 →
  自訂 → 光影 → 背景 → 鏡頭 → 比例 → 生成」的視覺順序；旅拍提前主風格與地點情境，
  雜誌提前封面與主題服裝，戰鬥學院則先呈現學校身份、制服與裝甲模組。
- **編輯視覺設計**：新增「影像處理」與「印刷與材質質感」兩個單選區，補足高對比棚拍、
  柔霧底片、黑白、雙色調、低飽和、亮面美妝，以及藝術紙、局部亮膜、金屬箔、膠印顆粒、
  孔版套色等編輯設計要素；預設套用、預覽腳本與 100 次模擬同步支援。
- **原則**：採 CSS 視覺排序保留既有 HTML 綁定與生成函式，避免大幅搬動資料區塊造成行為回歸。
- **驗證**：完成靜態檢查、預設引用檢查、提示詞預覽與 100 次隨機模擬。

## 2026-08-03　雜誌棚拍新增 03 服裝變化核心

- **改動**：`magazine.html` 在「02 主題 / 服裝方向」後新增「03 服裝變化核心」，
  分為胸口、腰側、肩部三組單選；原本「03 身形輪廓」及後續區塊順延至 04～13。
- **選項**：加入深 V、深 V至腰、交領深 V 結構胸衣、交叉綁帶深 V、深 V 大開襟；
  纏裙側開衩、高衩開裙、裙擺敞開；露肩剪裁、單肩斜裁、掛脖削肩。
- **生成邏輯**：預設三組皆為「無修改」，維持原本完整咒語輸出；選取後才加入
  `【服裝變化核心】` 與對應英文剪裁提示詞。一鍵預設會重置這三組，隨機套用會同步抽取。
- **驗證**：`check-static.mjs`、`validate-preset-refs.mjs`、`audit-100x.mjs`、
  `git diff --check` 全部通過。

## 2026-08-03　幻想廣告頁重新編號與服裝變化核心

- **改動**：`fantasy-fashion.html` 重新整理為 01～13 的固定順序，在 03「服裝輪廓」後加入
  04「服裝變化核心」；原本身形輪廓與後續區塊順延至 05～13。
- **選項**：服裝變化核心分為胸口、腰側、肩部三組單選，與雜誌棚拍頁使用同一套剪裁語意。
- **生成邏輯**：預設三組皆為「無修改」；選取後才加入 `【服裝變化核心】`，一鍵主題會重置，
  隨機套用會同步抽取，避免上一個畫面選擇殘留。
- **驗證**：重新檢查頁面區塊順序、編號提示、靜態結構、預設值與隨機生成流程。
