...
- 新增直排輪專屬動作、服裝、姿勢、光影、色彩與場景資料，涵蓋欄杆研磨、階梯飛越、城市加速、繞桿、牆面貼滑與濕地切入。
- 直排輪採開放式保護裝備與臉部完整可見規則，禁止全罩式頭盔、面罩、鏡面護目鏡遮擋眼鼻口；共用鎖臉、臉部幾何、真人骨架核心未修改。
- 同步更新隨機鏡頭池、模板數量門檻、主題分類與 100 次生成回歸驗證。

# 2026-09-04　v4.72 極限運動 44 組模板擴充

- 衝浪、滑雪、滑板、酷跑各新增 6 組完整一鍵模板，共新增 24 組，極限運動頁由 20 組擴充為 44 組；完成後衝浪／滑雪各 12 組、滑板／酷跑各 10 組。
- 新模板均回填作品概念、運動動作、服裝裝備、Layer、身形、姿勢、光影、色彩、背景、鏡頭、比例與強度；衝浪／滑雪持續沿用大尺度浪花／雪霧反應。
- `check-theme-classification.mjs` 的模板數量門檻更新為 44 組；既有分類、模板引用、UI flow、DOM 順序與隨機生成契約維持不變。

# 2026-09-04 v4.71 極限運動水花與雪霧尺度強化

- `extreme-sports.html` 新增頁面專屬 `environmentScaleData`；衝浪輸出高聳浪壁、寬幅白沫、前中景飛沫與懸浮水滴，滑雪輸出高而密的粉雪爆散、雪霧尾流與前景雪晶。
- 同步校準衝浪 6 組與滑雪 6 組動作、8 組水雪背景描述；衝浪／滑雪的一鍵模板與隨機套用都會依運動類別自動套用大尺度環境反應。
- 保留臉部、身體比例、板／器材接觸點與行進方向的可讀性；滑板與酷跑不加入水花或雪霧；未修改共用鎖臉、臉部幾何、真人骨架或光線核心。
- `audit-100x.mjs` 與主題分類檢查同步納入環境反應尺度區塊。

# 2026-09-04　v4.70 第五批風格範例分類與頁面專屬模板擴充

- **範圍**：盤點 `C:/Users/User/Desktop/ai生圖/風格範例` 的 49 張 JPG，建立唯一 primary owner 與機器可讀批次登記。
- **正式採用**：現代寫真新增 10 組、中式古典新增 4 組、西式婚紗新增 1 組、中式婚嫁新增 2 組、花卉甜美新增 2 組、日本和服新增 6 組；同步補入對應的服裝、材質／紋樣、配件、姿勢、互動、背景、光影與色彩資料。
- **保護**：幻想、動漫真人與雜誌素材只保留 record-only；來源身份、品牌／浮水印、可讀文字、固定身材與完整來源咒語不進正式頁或共用核心。`assets/core-prompt.js` 未修改。
- **驗證**：新增 `scripts/check-style-reference-batch-2026-09-04-fifth.mjs` 與 `docs/style-reference-batch-2026-09-04-fifth.md`；完成後執行批次、靜態、UI flow、preset、主題分類、DOM、預覽、隨機回歸與差異檢查。

## 2026-09-04　v4.69 全站工具頁流程順序回歸

- 依雜誌棚拍頁已確認的標準，檢查其他正式工具頁的「一鍵套用／隨機 → 生成完整咒語 → 輸出 → 一鍵複製」流程。
- 補齊 17 個欄式工具頁 `.wrap > .output-wrap` 的明確最終排序；旅拍既有的 `order:99` 保護規則保留，非欄式頁則維持自然 DOM 順序。
- `check-ui-flows.mjs` 新增欄式輸出定位契約，避免輸出區因 CSS order 漏設而跑到生成按鈕之前。
- 以中式古典與婚紗頁實際操作一鍵模板、生成、輸出與複製流程；全站靜態、UI、模板引用、DOM 順序、生成回歸與差異檢查通過。

## 2026-09-04　v4.68 雜誌棚拍介面流程標準化

- 將 `magazine.html` 的 00 一鍵模板、01～12 控制區與 13 生成區改用全站標準 `section-*` 區塊命名，整理 DOM 與 CSS order 契約。
- 00 區補上與其他工具頁一致的區段標題、說明文字與 `random-btn` 樣式；移除隨機按鈕的 inline style，並統一按鈕的 type 與圓角。
- 修正輸出框在 Flex 排序下未指定 order 的問題，固定為生成區之後，形成「套用／隨機 → 回填並生成 → 輸出 → 一鍵複製」的穩定流程。
- 保留雜誌頁 31 組一鍵模板、既有 Prompt 組裝、身份鎖定、臉部幾何與真人骨架核心；未改動主題資料內容。
- 更新 `check-ui-flows.mjs`、`reorder-dom-sections.mjs` 與雜誌工作流程文件；靜態、UI flow、模板引用、DOM 順序、瀏覽器一鍵生成與複製驗證通過。

## 2026-09-04　v4.67 極限運動高速攝影專題與全站發布前回歸

- 新增獨立 `extreme-sports.html`，專注衝浪、滑雪、滑板與酷跑四類極限運動；不納入重機賽車或遮臉頭盔主題。
- 新頁以高速快門凝結單一決定性瞬間，整合水花／雪霧、器材或場地接觸、低角度追拍、長焦壓縮與自然人體比例，提供 20 組完整一鍵模板。
- 新增首頁入口、主題 registry、正式頁清單、UI flow、DOM 順序、模板引用、分類驗證與 100 次隨機回歸覆蓋；共用鎖臉、臉部幾何、真人骨架與光線一致核心未修改。
- 雜誌頁移除一鍵模板生成後的特殊跳轉例外，與其他工具統一為套用後生成、顯示輸出區並定位至複製流程。
- 發布前驗證：24 個 HTML 頁面靜態檢查通過；23 頁標準 Prompt × 100 組共 2,300 組隨機回歸通過；模板、分類、DOM 順序、風格素材與差異檢查全部通過。

## 2026-09-04　v4.66 全站 Prompt 組裝回歸與雜誌模板精簡

- 檢查 22 個正式工具頁的實際組裝路徑，並執行 22 頁 × 100 組共 2,200 組隨機回歸；確認沒有 `undefined`、`NaN`、空白 Prompt 區段、身份核心缺失或相鄰重複行。
- 修正 `magazine.html` 的人物入鏡比例資料鍵：UI 使用的 `half_body`／`full_body` 原先對不到 `half_bodyShape`／`full_bodyShape`，導致構圖段落漏出空白；現已與介面值統一。
- 精簡雜誌與旅拍的服裝邏輯組裝，改為引用上方已輸出的主題與服裝方向，不再把完整 `themeText` 複製一次；花墨染分支同步處理。
- 精簡「白色毛料椅動感時裝」只保留獨有穿搭細節，背景、姿勢、光線、比例與精品語氣由各自正式控制區輸出；本地實際 Prompt 約由 9,856 降至 9,061 字元，且鎖臉、臉部幾何、真人骨架與光線一致核心完整保留。
- 強化 `scripts/audit-100x.mjs`：雜誌構圖 UI／資料鍵對照、空白區段與主題重複檢查納入全站回歸；`build-prompt-preview.mjs` 同步採用相同組裝規則。
- 驗證：`check-static.mjs`、`check-ui-flows.mjs`、`validate-preset-refs.mjs`、`build-prompt-preview.mjs`、`audit-100x.mjs` 與 `git diff --check` 全部通過。

## 2026-09-04　v4.65 兩組附圖分類與生活方式模板補強

- 本批兩組附圖分流為：雜誌棚拍的白色毛料椅丹寧時裝方向，以及 Luxury Lifestyle 的酒紅針織咖啡館夜景方向。
- 雜誌既有「白色毛料椅動感時裝」已承接第一組完整穿搭，並將一鍵模板的 `jewelry` 回填修正為實際耳飾選項，避免生成文字與 UI 選擇不一致。
- Luxury Lifestyle 新增精品咖啡館場景、酒紅細羅紋針織與駝色百褶短裙、咖啡桌邊坐姿、暖金散景、酒紅駝色黃銅配色與「酒紅咖啡館暖金夜景」一鍵模板。
- 分類紀錄見 [`docs/style-reference-expansion-2026-09-04-fourth.md`](style-reference-expansion-2026-09-04-fourth.md)。新增內容維持頁面專屬；共用鎖臉、臉部幾何與真人骨架核心未修改。

## 2026-09-04　v4.64 第三批風格範例分類與一鍵模板

- 讀取 `C:\Users\User\Desktop\ai生圖\風格範例` 目前 9 張 JPG；未發現對應咒語文字檔，依圖片主題建立匿名分類與單一 primary owner。
- 雜誌頁補強「白色毛料椅動感時裝」的完整丹寧穿搭，並新增「黑緞白棚美妝肖像」；新增幻想廣告「酒紅玫瑰哥德肖像」與「金色花拱高訂肖像」。
- 新增日本和服「絳紅金織花枝側影」、婚紗「粉花頭紗花園回眸」、旅拍「石階冰淇淋旅拍快照」、現代寫真「白裙石階自然寫真」與中式古典「花燈花卉漢服肖像」。
- 旅拍補入結構化丹寧襯衫短褲、菱格紋肩背包與冰淇淋互動選項；中式古典補入手提花燈與伸手觸燈姿勢；現代寫真補入木門石階場景與白色短洋裝。
- 分類紀錄見 [`docs/style-reference-expansion-2026-09-04-third.md`](style-reference-expansion-2026-09-04-third.md)。所有新增資料維持頁面專屬，不修改共用鎖臉、臉部幾何與真人骨架核心。

## 2026-09-04　v4.63 西式婚紗與中式婚嫁獨立工具

- 保留 `bridal-editorial.html` 為西式婚紗藝術寫真，明確維持婚紗、頭紗、教堂與海岸婚紗的既有頁面定位與資料。
- 新增獨立 `chinese-bridal.html` 中式婚嫁藝術寫真，提供龍鳳褂、秀禾服、鳳冠霞帔、明／宋／漢唐婚服、民國中式婚嫁與新中式高訂等 12 組完整一鍵模板。
- 中式婚嫁頁使用專屬婚服形制、工藝材質、儀式層、配件、姿勢、場景與色彩資料；不把西式婚紗、教堂長頭紗或仙俠／科技幻想正向語彙混入。
- 首頁、主題 registry、共用標準頁清單、DOM 順序、靜態檢查、UI flow、模板引用、分類檢查與 100 次隨機回歸均納入新頁；每個工具頁保留返回工具總覽連結。
- 共用鎖臉、臉部幾何、真人骨架、鏡頭、比例與光線一致核心未修改；新頁沿用既有核心組裝方式，不建立平行鎖臉系統。

## 2026-09-04 · v4.62 風格範例第二批擴充

- 依 16 張風格範例新增 7 組不重複的一鍵模板：婚紗荷塘白紗、中式朱紅紙傘宮牆、中式花燈糖葫蘆、雜誌酒紅拱廊、現代豹紋低角度、幻想粉羽花園與幻想黃色花卉鞦韆。
- 補入對應頁面的專屬場景、服裝、姿勢、道具、光影與色彩選項；追加方向中與既有模板重合的兩項以去重方式承接。
- 更新中式古典模板契約至 30 組，並修正所有新增模板的 UI 回填引用。
- 建立 [`docs/style-reference-expansion-2026-09-04-second.md`](style-reference-expansion-2026-09-04-second.md)，記錄正式採用、候選與不納入素材。
- 共用鎖臉、臉部幾何與真人骨架核心未修改。
- 驗證：`check-static.mjs`、`validate-preset-refs.mjs`、`check-ui-flows.mjs`、`build-prompt-preview.mjs` 通過。

## 2026-09-04　v4.61 三組候選風格正式主題化

- **正式新增三組完整模板**：Luxury Lifestyle「粉色薄紗窗邊 Luxury Lifestyle」、中式古典「古典華麗頭飾近距離肖像」、現代寫真「雨夜都市精品街拍」；各自串接場景、服裝、姿勢、互動、光影、配色、鏡頭與比例控制。
- **頁面專屬資料**：新增粉色薄紗窗景與全遮蔽內搭、華麗金工頭飾近肖像、雨夜精品街與透明雨傘等資料鍵；雨夜方向補上姿勢、鏡頭與光影隨機相容池。
- **品質與核心邊界**：薄紗模板保留完整內搭與可控材質層次；雨夜道具不含品牌或可讀文字；中式古典維持文化服裝語彙。共用鎖臉、臉部幾何、真人骨架與身份核心未修改。
- **分類紀錄**：先前三組候選轉為正式採用，並更新 `docs/style-reference-expansion-2026-09-04.md`；每組維持單一 primary owner。
- **驗證**：新增資料鍵與模板引用納入靜態、UI flow、模板、分類、Prompt 預覽與 100 次隨機回歸檢查。

## 2026-09-04　v4.60 四組風格範例主題化擴充與全站返回入口

- **正式新增四組模板**：現代寫真「校園綠蔭日常」、旅拍「白日車站針織行走」、中式古典「朱紅屏風古典華裳」、婚紗「紅玫瑰復古婚紗寫真」；每組均由場景、服裝、姿勢、互動、光影、配色與構圖欄位組成，不是單一長字串。
- **頁面專屬邊界**：新內容只進入各自頁面的資料表與模板物件；中式古典維持漢風／盛唐／宋韻主軸，不加入仙俠、法術或科技幻想語彙。
- **全站導覽**：22 個正式工具頁補上「返回工具總覽」至 `index.html` 的連結；首頁維持總覽入口，不新增自返按鈕。
- **核心邊界**：共用鎖臉、臉部幾何、真人骨架與身份核心未修改。
- **驗證**：靜態、模板引用、UI flow、DOM 順序、分類登記、Prompt 預覽與 2,100 組隨機回歸全部通過。

## 2026-09-03　v4.59 日本和服象牙金粉深 U 高訂模板

- **和服頁專屬資料**：新增象牙金粉改良和服主題、金線立體花卉刺繡、花卉蕾絲鏤空材質與象牙／淡金／柔粉搭配色。
- **服裝改造核心**：新增深 U 型高訂和服領口與花卉鏤空腰側覆層；肩袖沿用既有薄紗袖覆層，並保留和服包襟、腰帶與振袖結構。
- **構圖與模板**：新增大腿以上中近景與略微側身凝視姿勢，建立「象牙金粉深 U 高訂和服」完整一鍵模板，使用既有町家木造室內、障子窗光、斜側角度與 9:16 比例。
- **核心邊界**：不固定加入 85mm、來源人物外貌、髮色／膚色或額外身份描述；共用身份、臉部幾何、真人骨架、光線一致性與負面核心未修改。
- **驗證**：上線前執行靜態、UI flow、DOM 順序、模板引用、主題分類、固定預覽、100 次隨機回歸與差異檢查。

## 2026-09-03　v4.58 四頁風格範例擴充

- **現代寫真**：新增高樓窗光、清水模階梯夜景、精品店門口與血橙果園四個頁面專屬場景，並補入相符服裝、姿勢、前景互動、光影與配色；新增 4 組完整一鍵模板。
- **旅拍**：新增車站月台與地中海彩牆主題快選、車站環境人物構圖與車站夕陽逆光；新增金色夕陽車站回眸、地中海彩牆草帽 2 組完整一鍵模板。
- **Luxury Lifestyle**：新增海景高樓陽台、橄欖綠度假服裝、海岸清澈日光與海藍橄欖白配色；新增 1 組完整一鍵模板。
- **雜誌棚拍**：新增白色毛料椅極簡棚、白椅甩髮動感姿勢與白椅高亮棚光；新增 1 組完整一鍵模板。
- **分類與核心邊界**：新增 `docs/style-reference-expansion-2026-09-03.md`；素材只進入唯一 primary owner 的頁面資料，未修改共用身份、臉部幾何、真人骨架或鎖臉核心，也未採用品牌、浮水印、來源身份與固定身材描述。

## 2026-09-02　v4.57 金色花田紅紗藝術旅拍模板

- **旅拍模板**：新增「金色花田紅紗藝術旅拍」完整一鍵模板，固定花田中景藝術肖像、自然回眸站姿、黃金時刻、旅遊雜誌媒材、2:3 比例與旅拍自然生活感。
- **頁面資料**：新增旅拍專屬「紅色絲質薄紗披覆」服裝方向與「70mm 中景人像」鏡頭；以受控層疊、穩定包覆與真實布料重量描述取代模糊的自由披覆，避免服裝結構漂移。
- **核心邊界**：只在 travel.html 的頁面資料中加入花田、紅紗、膠片色彩與中景攝影語彙；共用身份、臉部幾何、真人骨架、光線一致性與負面核心未修改。
- **驗證**：通過靜態語法、UI flow、DOM 順序、模板引用、主題分類、風格範例批次檢查、固定 Prompt 預覽、2,100 組隨機回歸與差異檢查。

## 2026-09-02　v4.56 第二批 22 張風格範例分類與主題化擴充

- **素材盤點**：重新讀取 C:/Users/User/Desktop/ai生圖/風格範例，確認第二批有 22 張圖片，沒有同批提示詞文字檔；建立獨立匿名來源登記與每筆單一 primary owner。
- **分類分流**：4 張歸入現代寫真、2 張歸入 Luxury Lifestyle、1 張歸入雜誌、4 張歸入旅拍、6 張歸入幻想廣告、2 張歸入動漫角色、1 張歸入韓系偶像、1 張歸入公仔，1 張列為中式古典候選。
- **正式採用**：現代寫真新增茶飲店生活快照；旅拍新增海岸單車花漾旅拍；幻想廣告新增水晶碎片美妝特寫、紅紗風場高訂主視覺、水墨書法流動禮服，共 5 組完整一鍵模板。新模板重用既有資料鍵，沒有建立平行核心。
- **保留與排除**：已存在或高度重複的生活方式、雪景和風、美妝、舞台、IP、品牌、可讀文字、來源水印與單張身材／服裝特徵只記錄，不轉成共用選項或正向核心；中式旗袍素材保留候選，不改變中式古典頁的漢風、盛唐、宋韻與新式漢服主軸。
- **核心邊界**：assets/core-prompt.js 未修改；身份鎖定、臉部幾何、真人骨架、光線一致性與負面核心維持原版本。
- **驗證**：新增 scripts/check-style-reference-batch-2026-09-02-second.mjs；第二批專用檢查、靜態、UI flow、preset 引用與 git diff --check 均通過，後續上線前再完成固定預覽與 100 次隨機回歸。

## 2026-09-02　v4.55 38 張風格範例分類與婚紗頁擴充

- **素材盤點**：重新讀取 C:/Users/User/Desktop/ai生圖/風格範例，確認本批次有 38 張圖片，沒有同批提示詞文字檔；建立匿名來源登記與每筆單一 primary owner。
- **分類分流**：26 張歸入婚紗藝術寫真、6 張歸入現代寫真、1 張歸入寫真旅拍、4 張歸入雜誌棚拍、1 張列為中式古典候選；不將單張素材跨頁複製。
- **婚紗正式採用**：新增海岸藍天日光背景、海岸明亮日光、背影回眸長紗構圖、背影回眸展示頭紗，以及海岸明亮日光長紗、背影回眸長紗主視覺、落地窗柔霧蓬裙婚紗 3 組完整一鍵模板。
- **核心邊界**：來源人物身份、外貌、品牌／浮水印文字、固定身材比例與整段來源咒語不進共用核心；assets/core-prompt.js 未修改。
- **驗證**：新增 scripts/check-style-reference-batch-2026-09-02.mjs，與既有靜態、UI、preset、主題分類、固定預覽、隨機回歸及差異檢查一併作為上線驗收。

依時間排序的完整開發記錄。`CLAUDE.md` 只放最新現況摘要，詳細歷史都在這裡；
需要追溯「某個功能是哪一批加的、為什麼這樣改」時查這份文件。

## 2026-09-02　v4.54 142 張風格範例分類與頁面整合

- **素材登記**：讀取 `C:\Users\User\Downloads\風格範例_142張逐張咒語分析.xlsx` 的 142 筆資料，建立 `assets/style-reference-142.js` 匿名 row 登記；五個來源攝影群組各自保留來源統計，每筆資料只指定一個 primary owner。
- **主題分流**：以主要作品敘事分流到現代寫真、旅拍、雜誌、Luxury Lifestyle、名媛、節慶、花漾、幻想與動漫等正式頁；跨群組素材以明確 override 記錄，不把同一筆資料重複放入多個 preset pool。
- **頁面擴充**：現代寫真由 15 組增至 20 組模板，旅拍由 14 組增至 18 組，雜誌由 24 組增至 28 組，Luxury Lifestyle 由 10 組增至 11 組；新增內容均是頁面專屬場景、服裝、姿勢、互動、光影、色彩、鏡頭或完整作品模板。
- **分類修正**：`festival-editorial.html` 補入 `assets/theme-registry.js` 與分類驗證；節慶、特殊角色、商品展示與藝術黑墨素材不跨頁污染其他工具。
- **核心邊界**：不將來源人物外貌／身份、固定數字比例、特定髮色／唇色／妝容、貓耳、節日道具或完整長篇攝影核心加入 `assets/core-prompt.js`；共用鎖臉、臉部幾何、真人骨架與光線一致性文字維持 v4.52。
- **驗證**：新增 `scripts/check-style-reference-142.mjs`，並通過靜態、UI flow、preset 引用、主題分類、固定 Prompt 預覽、隨機稽核與 `git diff --check`；原始工作簿與私人來源素材不進 Git。

## 2026-09-02　v4.53 風格範例分流與現代寫真模組擴充

- **素材分流**：依風格範例的主要敘事，將玫瑰木蝴蝶薄紗、藍白格紋臥室、灰藍蕾絲、粉色羅紋針織與淡紫緞面窗邊歸入 `modern-portrait.html`；白襯衫白磚棚拍更新既有雜誌模板；香檳銀夜景歸入 `gala-socialite.html`；飯店長廊長裙回眸歸入 `travel.html`。
- **現代寫真擴充**：新增 3 個頁面專屬室內場景、4 個服裝方向、5 個床面／地面姿勢、3 個光影選項、1 組玫瑰木搭配色與 2 個頁面專用鏡頭語言；完整一鍵模板由 10 組擴充為 15 組。
- **其他主題同步**：雜誌新增白磚高亮棚拍背景並強化既有「極簡白襯衫封面」；名媛新增香檳銀晚宴禮服、夜景人物美光、夜景散景鏡頭與模板；旅拍新增金色長廊窗光、長廊透視鏡頭與模板。
- **核心邊界**：聖誕紅緞面保留給節慶專題；來源地域外貌、髮色／唇色／妝容、來源身份、固定身材數字、貓耳與完整長篇攝影核心均未加入共用核心或現代頁。`assets/core-prompt.js` 文字維持 v4.52 基準。
- **驗證契約**：分類檢查更新為現代寫真 15 組完整模板；所有新增值均由既有模板引用驗證與隨機回歸覆蓋。
...
- **主題歸屬**：分類為「寫真旅拍」，不併入日本和服美學頁；重點是雪景溫泉溪谷、深色木造建築、冷白雪光與環境人物構圖。
- **服裝資料**：新增「雪景花卉包覆和風長袍」手動選項，描述白色／冰藍灰花卉、自然交疊 V 形前襟、腰間繫帶、長袖、層疊裙擺與真實布料重量、張力、皺褶及垂墜。
- **一鍵模板**：新增「一鍵套用｜雪景溫泉花卉和風旅拍」，完整回填旅拍風格、全身環境構圖、2:3 比例、三分之四側視角、冷白自然光、靜態姿勢、編髮、小型花飾與雪景主題描述。
- **核心邊界**：新內容只接入旅拍主題資料與模板，不複製或改寫共用身份鎖定、臉部幾何、真人骨架、光線一致與負面核心；旅拍既有不使用 `bodyShape` 的契約維持不變。
- **資產管理**：來源圖片只作為分析參考，沒有複製到正式網站或提交私人素材。

## 2026-08-30　v4.50 共用身材控制新增深溝豐滿上圍選項

- **共用身材控制**：在 16 個含 `bodyShape` 的標準人像工具頁同步新增「深溝豐滿上圍身材」；旅拍、娃娃轉換、店家廣告與編輯視覺設計沒有此控制軸，因此維持原有頁面契約。
- **生成語意**：新增 Prompt 聚焦成人女性、自然豐滿上圍、自然深溝、定義腰線、真實解剖、自然重力與穩定胸腰比例；服裝剪裁仍由各頁服裝與服裝改造核心控制，並排除過度誇張與人工塑形。
- **核心邊界**：未修改 `assets/core-prompt.js` 的鎖臉、臉部幾何、真人骨架或光線一致性核心；不新增平行身份核心。
- **驗證契約**：`check-ui-flows.mjs` 將 `full_bust_cleavage` 納入共用身材選項檢查；隨機稽核透過頁面現有 `BODY_SHAPES`／`bodyShapes` 動態選項池自然涵蓋新值。

## 2026-08-26　v4.49 中式青春古風內襦層次與仙俠醉仙模板

- **中式古典服裝**：新增「敞襟外袍・內襦層疊」服裝主題與「低方領貼身內襦」胸口局部改造；英文資料明確要求不透明內襦、兩層服裝結構、自然布料張力、皺褶與重力垂墜。
- **青春古風模板**：新增「青春古風・敞襟內襦」一鍵模板，組合既有青春語氣、古街動態、明亮散射光與桃粉嫩玉月白配色，不建立新的共用人物核心。
- **仙俠模板**：新增「舉壺微醺動勢」姿勢與「醉仙・月下狂飲」一鍵模板；重點是自然重心偏移、玉壺、袖帶／裙襬多方向動勢與月下全身構圖，不加入跌倒或誇張肢體變形。
- **核心邊界**：沒有把完整 `Adult woman`、年齡數字、眼神或身形長段落複製進鎖臉核心；既有 `bodyShape`、身份、臉部幾何與真人骨架核心維持原樣。
- **驗證同步**：中式古典一鍵模板契約更新為 26 組、服裝主題更新為 19 套；固定 Prompt 預覽、隨機稽核與主題化服裝改造契約同步納入新值。
...
- **主題語氣修正**：將花卉甜美、晚宴名媛、韓系偶像、古代女神、花仙子、仙俠、動漫角色、異世界與戰鬥學院的
  `intensity` 泛用材質／爆發／粒子字串改為各主題對應的造型、動態、舞台、儀式或世界觀語彙；介面中文標籤與選項數量不變。
- **跨主題稽核**：`check-ui-flows.mjs` 新增強度資料契約，檢查生成 select 與 `themeTemplates` 同步採用主題詞，
  阻止泛用強度詞重新流入不相符的主題頁。
- **DOM 順序**：新增 `scripts/reorder-dom-sections.mjs`，把 13 頁的 section 依實際決策順序排列；`check-ui-flows.mjs`
  改檢查 DOM 先後，不再以 CSS `order` 作為唯一真實順序。雜誌頁補回遺漏的 `</section>`，避免主題區錯誤嵌入風格區。
- **範圍保護**：第 6 項「選項分級顯示」完全不改；選項、Layer、身形、比例、鎖臉、臉部幾何與真人骨架核心均保留原契約。
- **驗證**：靜態檢查、UI flow、DOM 順序、preset 引用、固定 Prompt 預覽、1,900 組隨機稽核與 `git diff --check` 全部納入上線前檢查。

## 2026-08-04　v4.43 編輯視覺設計 Stage 2 重整

- **版型減法**：手動版型由 31 個與一鍵模板重疊的選項收斂為 14 個結構家族；31 組一鍵模板、六分類與完整 preset 回填保留。
- **平面設計邊界**：編輯頁不再套用會暗示重新生成人像的通用攝影輸出／負面核心，改為專屬來源照片鎖定、景別保護與非破壞性編輯規則。
- **控制精簡**：新增「無主圖形」，移除重複的照片黑白色彩入口與等效印刷質感；色彩系統只控制文字與圖形，影像處理才控制整體照片色調。
- **文字可靠性**：使用者填寫的文字原樣使用，未填欄位保持空白；日期、座標、品牌、成分、署名等事實資料不得自行杜撰，全部留空時最多補一個非事實性短標題。
- **驗證**：`check-static.mjs`、`check-ui-flows.mjs`、`validate-preset-refs.mjs`、`build-prompt-preview.mjs`、`audit-100x.mjs` 與 `git diff --check` 全部通過；全站 19 頁 × 100 組共 1,900 組模擬為 0 issue。

## 2026-08-04　v4.42 婚紗藝術寫真 Prompt 去重瘦身

- **縮短重複描述**：婚紗頁改用短版高訂核心、反廉價限制與輸出品質描述，移除與 `editorial-finish.js` 重疊的通用精品完成度、材質與負面限制段落；頭紗仍保留臉部幾何與同場光線保護。
- **鎖臉不縮減**：身份鎖定、臉部幾何、真人骨架、姿勢、光線與構圖核心未改動；`build-prompt-preview.mjs` 顯示共用核心總長度維持 6,381 字元。
- **結果與驗證**：標準婚紗 Prompt 由 10,318 降至 8,870 字元（-14.0%）；黑金與珍珠灰模板分別為 8,899／8,780 字元，均低於 9,000。預覽與 100 次隨機稽核同步改採婚紗專屬精品核心檢查。

## 2026-08-04　v4.41 色彩系統搭配色組與 AI 主題配色

- **共用 AI 配色提示**：`assets/core-prompt.js` 新增 `HB_CORE_PROMPT.controls.autoColor`，依選定主題、服裝、材質、光線與背景選擇主色、輔色與克制點綴色；不改身份鎖定與臉部幾何核心。
- **主題搭配色組**：中式古典、日本和服、韓國韓服、婚紗藝術寫真與編輯視覺設計的色彩系統各新增 5 組主題化搭配色，並保留原有色彩選項與自訂配色優先規則。
- **介面與驗證契約**：各頁新增 `aiThemeScenePalette` 選項；`check-ui-flows.mjs` 會檢查 AI 選項、可見文案與至少 5 組搭配色，生成與隨機稽核沿用現有色彩資料來源。

每筆記錄盡量只留：**做了什麼 → 為什麼 → 驗證方式**，實作細節（哪一行、哪個函式）
留在對應的 commit diff 或 `docs/history/` 底下的原始交接文件。

## 2026-08-04　v4.40 四頁精品質感提升與中式青春古風擴充

- **精品成像共用核心**：新增 `assets/editorial-finish.js`，由中式古典、
  日本和服、韓系偶像、婚紗藝術寫真四頁共用高預算編輯成像規則；補足主體層級、方向性光影、
  材質重量、縫線與工藝可信度，並排除扁平電商光、租借服、塑膠裝飾、印刷感刺繡與過度堆景。
- **主題化精品方向**：四頁各新增 2 組完整精品模板與 1 個「隨機套用｜精品主視覺」按鈕，
  模板透過既有 `themeTemplates`／`applyThemeTemplate` 完整回填，不改身份鎖定、臉部幾何或真人骨架核心。
- **中式青春古風**：中式古典頁新增 3 組畫面語氣、5 組動態姿勢、春日／古街／飄帶髮絲 3 組
  光影配色路線，以及 5 組青春古風一鍵模板；不加入年齡數字或幼態描述，保留成人身份與原有頁面架構。
- **預覽與稽核**：`build-prompt-preview.mjs` 同步輸出四頁精品模板；`audit-100x.mjs` 對四頁實際
  檢查精品核心是否進入生成結果。
