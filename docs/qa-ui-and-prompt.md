...
- 直排輪模板與隨機生成均使用專屬動作、服裝、姿勢、光影、色彩、背景與鏡頭池，避免誤帶水上或雪地專用控制。
- 直排輪輸出包含臉部完整可見規則，只允許開放式護具，禁止全罩式頭盔、面罩與鏡面護目鏡遮擋眼鼻口；共用身份鎖定與真人骨架核心仍保留。
- `check-static.mjs`、`check-ui-flows.mjs`、`validate-preset-refs.mjs`、`check-theme-classification.mjs`、`reorder-dom-sections.mjs --check`、`build-prompt-preview.mjs`、`audit-100x.mjs 100` 與 `git diff --check` 應全部通過。

## 2026-09-04 v4.72 極限運動一鍵模板擴充回歸

- 極限運動頁新增 24 組完整一鍵模板；衝浪／滑雪各 12 組、滑板／酷跑各 10 組，總數 44 組。
- 每組模板均通過欄位引用檢查，包含作品概念、動作、服裝裝備、Layer、姿勢、光影、色彩、背景、鏡頭、比例與強度。
- 主題分類檢查同步驗證四類運動的模板數量，避免新增模板集中到單一運動。

## 2026-09-04 v4.71 極限運動環境反應尺度回歸

- 衝浪與滑雪各自有頁面專屬環境反應尺度；衝浪可產生大浪壁、寬幅浪花與前景飛沫，滑雪可產生高而密的粉雪 plume、雪霧尾流與前景雪晶。
- 環境反應尺度依 `sport` 條件組裝，滑板與酷跑不會誤帶水花或雪霧；臉部、器材接觸點與運動方向保護文字仍保留。
- `check-theme-classification.mjs`、`audit-100x.mjs` 與靜態／UI／模板／DOM 檢查需確認新區塊存在且不產生空白段落。
...
- 新模板固定自然回眸站姿、黃金時刻、旅遊雜誌媒材、2:3 比例與旅拍自然生活感，沒有把來源人物外貌或身份特徵寫入共用核心。
- 靜態、UI flow、DOM 順序、模板引用、主題分類、風格範例批次、固定預覽、2,100 組隨機回歸與 git diff --check 均通過。
...
- 現代寫真 10 組一鍵模板完整回填場景、服裝、改造 Layer、身形、姿勢、互動、光影、配色、鏡頭、比例與畫面強度；實際生成 Prompt 也會輸出選定的身形輪廓。
- `Luxury Lifestyle` 維持獨立頁；現代寫真不引用其主題資料，兩頁只共享正式共用核心與欄位契約。
- 全站主題登記表與分類規範通過：每一正式頁都有唯一主要歸屬，使用者指定排除的風格特定詞未進入共用核心。
- 靜態、UI flow、DOM 順序、preset 引用、固定預覽、分類檢查、21 頁 × 100 組隨機稽核與 `git diff --check` 應全部通過，0 issue。
