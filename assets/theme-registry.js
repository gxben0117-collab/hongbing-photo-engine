(function () {
  const registry = {
    ...
    'travel.html': {
      ...
      owns: ['地點', '旅拍情境', '旅行動態', '戶外光線', '車站月台', '地中海彩牆與海岸度假情境'],
      allowed: ['真實城市、自然景點、交通與旅途中可觀察的生活互動', '車站月台透視、彩色海岸街巷與草帽度假造型'],
    ...
    'magazine.html': {
      ...
      owns: ['封面語氣', '主題服裝', '妝容', '棚拍光線', '封面構圖', '白色雕塑毛料椅棚景', '受控甩髮動態'],
      allowed: ['高級時裝、精品美妝、封面景別、棚拍背景與編輯表情', '高亮白棚、雕塑家具與現代動態時裝肖像'],
    ...
    'luxury-lifestyle.html': {
      ...
      owns: ['住宅空間', '沙發', '咖啡桌', '床上', '窗前', '門口', '庭院', '陽台', '海景高樓陽台', '吧檯', '生活互動'],
      allowed: ['家具尺度、室內材質、住宅品牌主視覺、生活道具', '海景高樓住宅與高級度假生活方式'],
    ...
    'modern-portrait.html': {
      ...
      owns: ['現代畫面語氣', '臥室', '沙發客廳', '日式室內', '走廊', '戶外綠意', '高樓窗光', '清水模階梯', '精品店門口', '血橙果園', '姿態攝影', '現代服裝'],
