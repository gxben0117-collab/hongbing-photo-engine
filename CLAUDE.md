...
- 目前版本：v4.61；共用身份／臉部幾何核心基準仍為 v4.3，共用核心文字基準仍為 v4.52。
...
- **三組候選風格正式主題化**（2026-09-04，v4.61）：Luxury Lifestyle 新增粉色薄紗窗邊、現代寫真新增雨夜都市精品街拍、中式古典新增古典華麗頭飾近距離肖像；各模板使用頁面專屬場景、服裝、姿勢、互動、光影、配色與鏡頭資料，共用鎖臉、臉部幾何、真人骨架與身份核心未修改。完整分類紀錄見 [`docs/style-reference-expansion-2026-09-04.md`](docs/style-reference-expansion-2026-09-04.md)。

- **四組風格範例主題化擴充**（2026-09-04，v4.60）：現代寫真新增校園綠蔭日常、旅拍新增白日車站針織行走、中式古典新增朱紅屏風古典華裳、婚紗新增紅玫瑰復古婚紗寫真；同步補上所有 22 個正式工具頁的返回工具總覽連結。各模板使用頁面專屬資料與既有控制軸，共用鎖臉、臉部幾何與真人骨架核心未修改。完整分類紀錄見 [`docs/style-reference-expansion-2026-09-04.md`](docs/style-reference-expansion-2026-09-04.md)。

- **日本和服深 U 高訂模板**（2026-09-03，v4.59）：`japanese-kimono.html` 新增象牙金粉改良和服、金線立體花卉刺繡、花卉蕾絲鏤空、深 U 型高訂領口、花卉鏤空腰側覆層、大腿以上中近景與側身凝視姿勢，形成 1 組完整一鍵模板；沿用既有日本和服核心與相機／比例控制，共用鎖臉、臉部幾何與真人骨架核心未修改。

- **四頁風格範例擴充**（2026-09-03，v4.58）：依本批參考素材將高樓窗光、清水模階梯、精品店、血橙果園、車站月台、地中海彩牆、海景高樓陽台與白色毛料椅棚景分流至各自工具頁，新增 8 組完整一鍵模板；未修改共用鎖臉、臉部幾何或真人骨架核心。

- **金色花田紅紗藝術旅拍**（2026-09-02，v4.57）：旅拍新增完整一鍵模板、花田中景藝術肖像構圖、70mm 中景鏡頭與紅色絲質薄紗披覆服裝方向；僅使用旅拍頁專屬資料，共用鎖臉、臉部幾何與真人骨架核心維持不變。

- **第二批 22 張風格範例分類與主題化擴充**（2026-09-02，v4.56）：建立獨立匿名來源登記與專用驗證；正式新增現代寫真茶飲店快照、旅拍海岸單車花漾，以及幻想廣告水晶碎片美妝、紅紗風場、水墨書法流動禮服 5 組完整模板。其他素材只保留分類紀錄或中式古典候選，不修改共用核心。

- **38 張風格範例批次分類與婚紗擴充**（2026-09-02，v4.55）：建立匿名來源登記、單一 primary owner 與批次驗證；正式採用海岸藍天日光、海岸明亮日光、背影回眸長紗構圖、背影回眸展示頭紗與 3 組婚紗一鍵模板。現代寫真、旅拍、雜誌與中式古典素材只記錄或保留候選，不改共用核心。

- **142 張風格範例分流與頁面模組擴充**（2026-09-02，v4.54）：以 `assets/style-reference-142.js` 登記 142 筆匿名來源 row，為每筆資料指定單一 primary owner，並以 `scripts/check-style-reference-142.mjs` 驗證覆蓋、分流與特殊案例。現代寫真、旅拍、雜誌與 Luxury Lifestyle 分別擴充至 20、18、28、11 組完整一鍵模板；節慶頁補入正式主題登記。未修改共用鎖臉、臉部幾何或真人骨架核心。

- **風格範例分流與頁面模組擴充**（2026-09-02，v4.53）：現代寫真擴充玫瑰木、藍白格紋、灰藍蕾絲、粉色羅紋與淡紫緞面頁面資料，完整一鍵模板由 10 組增至 15 組；雜誌更新既有白襯衫模板與白磚背景，名媛新增香檳銀夜景，旅拍新增金色長廊回眸。聖誕、貓耳、來源人物外貌／身份與固定數字比例不加入共用核心；共用核心文字維持 v4.52。

- **現代寫真攝影與全站分類規範**（2026-09-01，v4.52）：新增獨立 `modern-portrait.html`，提供現代生活場景、姿態、服裝、光影、色彩、鏡頭與 10 組完整一鍵模板；新增 `assets/theme-registry.js`、全站分類文件與分類驗證腳本。使用者指定不採用的東亞外觀、特定髮色／唇色／身材、單一色彩與動物耳朵限制未寫入共用核心。

- **雪景溫泉花卉和風旅拍**（2026-08-31，v4.51）：依兩張風格範例新增旅拍主題選項、白藍灰花卉包覆式和風長袍服裝方向與完整一鍵模板；吸收雪景、木造溫泉溪谷、自然交疊前襟、腰間繫帶、編髮花飾與冷白自然光語彙。未修改共用鎖臉、臉部幾何或真人骨架核心。

- **共用身材控制新增**（2026-08-30，v4.50）：16 個標準人像頁同步加入「深溝豐滿上圍身材」；生成描述保留成人、自然重力、真實解剖與胸腰比例限制，服裝剪裁仍由各頁服裝控制。未修改共用鎖臉、臉部幾何或真人骨架核心。

- **全站 Prompt 核心去重與回歸**（2026-08-22，v4.48）：標準人物頁的共用負面段落不再重複真人骨架已涵蓋的
  解剖限制；店家廣告的人像主視覺改用完整的 `CORE_PERSON_HERO_NEGATIVE_PROMPT`。Luxury Lifestyle 移除
  重複的全域負面核心，身份優先段只組入一次；20 頁靜態、UI、DOM、模板引用與 2,000 組隨機稽核全部通過。

- **Luxury Lifestyle 獨立工具頁**（2026-08-22，v4.47）：新增 `luxury-lifestyle.html`，提供沙發、咖啡桌、床上、窗前、門口、庭院、陽台、吧檯、餐桌與衣帽間等生活場景，以及 10 組完整一鍵模板；雜誌頁原先誤放的 Luxury Lifestyle 公寓模板已移除。新頁只沿用共用人像核心組裝方式，不建立平行鎖臉核心。

- **全站主題契約與 DOM 順序整理**（v4.44）：修正花卉、晚宴、偶像、女神、仙俠、動漫、異世界與戰鬥學院的
  泛用強度詞，改為各主題的動態、材質與畫面語氣；13 頁改為實際 DOM 依決策順序排列，雜誌頁補回遺漏的
  `section` 關閉標記。第 6 項選項分級顯示、選項數量、鎖臉／臉部幾何／真人骨架核心均未修改。

- **編輯視覺設計 Stage 2 重整**（2026-08-04，v4.43）：手動版型由 31 個重複模板型選項收斂為 14 個結構家族，
  31 組一鍵模板與分類契約保留；新增無主圖形、精簡印刷質感與照片色彩選項，並改用專屬來源照片鎖定、
  原圖景別保護、文字只採用已提供內容與非事實性標題規則，不再套用可改人物的通用人像輸出／負面核心。

- **婚紗 Prompt 去重瘦身**（2026-08-04，v4.42）：婚紗頁改用短版高訂核心、反廉價限制與輸出品質，移除和 `editorial-finish.js` 的重複段；標準輸出從 10,318 降至 8,870 字元，鎖臉、臉部幾何、骨架、姿勢與光線核心完全保留。

- **色彩系統搭配與 AI 配色**（2026-08-04，v4.41）：中式古典、和服、韓服、婚紗與編輯視覺設計各新增 5 組主題搭配色，並共用 `HB_CORE_PROMPT.controls.autoColor`；色彩只控制主色、輔色與點綴色，不改身份、臉部幾何或真人骨架。

- **全站身份優先與 Prompt 瘦身**（2026-08-04，v4.37）：共用核心在所有正式人像輸出的可變內容之後
  加入短版最終身份優先保護；幻想、仙俠、雜誌、中式古典、和服、韓服與婚紗移除身形／身份重複片段，
  保留 `Japanese Curvy Slim-Waist Adult Female Silhouette` 特例。婚紗隨機材質至少保留 1 項，Layer
  改為尊重使用者選擇；回歸腳本新增最終身份保護與婚紗 Layer 契約。

- **全站人像控制契約**（2026-08-04，v4.36）：以幻想廣告頁為共同範本，正式人像頁共用拍攝角度、
  圖片比例、身形輪廓與服裝改造 Layer；旅拍、雜誌、婚紗及傳統服飾頁保留必要的主題專屬追加，
  不改動共同控制值。`check-ui-flows.mjs` 已加入共用契約檢查。
- **婚紗頁鏡頭與排版修正**（2026-08-04，v4.36）：移除 `85mm 婚紗人像`／`bridal85mm`，
  改用幻想頁共用鏡頭與比例資料；05 服裝改造核心的胸口、腰側、肩部改為響應式三區網格，
  並納入模板、預覽與 100 次隨機回歸。

- **婚紗藝術寫真工具頁**（2026-08-03，v4.35）：新增獨立 `bridal-editorial.html`，提供 12 組婚紗模板、婚紗輪廓、材質、服裝改造 Layer、頭紗、配件、妝髮、姿勢、光影、背景與鏡頭控制；頭紗身份保護採婚紗頁條件式模組，不改全站身份核心。

- **服裝改造核心主題化擴充**（2026-08-03，v4.34）：14 個正式服裝改造頁新增主題專屬胸口、腰側、
  肩部細節與相容隨機池；`assets/garment-core.js` 統一 Layer 解析、手動保留、未選部位補足與片段去重，
  每頁至少 3 組一鍵模板改用主題選項，回歸契約覆蓋 UI、資料、隨機池與 preset。

- **唯美古風寫真擴充**（2026-08-03，v4.33）：`chinese-classical.html` 將新式古風整理為唯美古風，
  服裝擴充為四組十七套，新增古風宮廷華服、古風薄紗華服、荔枝宮苑、白玉御苑湯池等支援資料，
  並加入 6 組完整一鍵模板；正向 Prompt 維持中國古典寫真語彙，不混入仙俠、花仙子或西式幻想。
- **全站介面文案關聯性回歸**（2026-08-03）：檢查 19 個正式工具頁與首頁；主題材質標題已依
  神話、動漫、奇幻、花卉、晚宴、異世界、偶像、仙俠與傳統服飾分別命名，並修正複製而來的
  服裝示例、坐姿情境與首頁數量描述。`check-ui-flows.mjs` 現在會阻止泛用殘留文案回歸。
- **編輯視覺設計 31 組模板擴充**（2026-08-03，v4.32）：`editorial-identity.html` 的一鍵模板
  分為時尚編輯 6、美業廣告 4、動漫電玩 4、電影海報 7、寫真書 5、旅遊設計 5；不含運動競技、
  犯罪、恐怖或心理驚悚方向，也不提供分類隨機模板。新增文案語氣、輔助標記、metadata、credit
  line 與 31 組完整 preset 回填，Stage 2 生成仍只做非破壞性排版與圖形後製。
- **編輯視覺設計驗收契約**（2026-08-03，v4.32）：UI flow、preset 引用、100 次隨機稽核與固定
  Prompt 預覽會逐一覆蓋全部 31 組模板，並檢查模板按鈕與資料池一一對應、來源照片鎖定與事實
  資訊不得自行捏造。
- **服裝改造核心 Layer 全站同步**（2026-08-03，v4.31）：所有具胸口、腰側、肩部三區改造的
  正式頁都提供 Layer 0／3／6／9／隨機 Layer。Layer 只控制隨機套用時要補足的改造部位數，
  不覆蓋手動已選部位，也不寫入生成 Prompt；中式、和服、韓服模板會依實際改造數回填 Layer。
- **新增中式古典美學工具頁**（2026-08-03）：新增 chinese-classical.html，以幻想廣告頁的
  UI 與資料組裝結構為基底，獨立提供漢風、盛唐、宋韻、唯美古風四組十七套服裝主題；材質最多兩項，
  江南水墨只屬背景，正向 Prompt 不混入仙俠或科技幻想語彙。
- **中式古典美學控制軸擴充**（2026-08-03）：04 拆為材質／紋樣最多 2 個與中式飾品單選，
  09 拆為光影與色彩系統，10 只保留背景場景；補充服裝展示構圖、自然古典姿勢、
  18 組模板欄位與 100 次稽核資料。
- **全專案 UI、生成咒語與一鍵設定回歸檢查**（2026-08-03）：新增
`scripts/check-ui-flows.mjs`，靜態檢查 19 個工具頁的必要控制項、radio 初始值、
...
- **移除三頁留白控制**（2026-08-03）：`chinese-classical.html`、`japanese-kimono.html`、
  `korean-hanbok.html` 的 10B 留白方向已移除；背景場景成為 10 的唯一控制，資料、模板、
  隨機套用與生成輸出同步收斂。
- **三頁介面重整與骨架核心強化**（2026-08-03，v4.30）：三頁的自訂服裝、材質／紋樣、姿勢、
  配色與背景欄位移回對應控制區，08 僅保留畫面強度與其他要求；背景新增無背景純白色。
  只修改 `assets/core-prompt.js` 的 `CORE_REALISTIC_ANATOMY`，補上連續頭頸肩脊椎結構與
  非合成臉部約束，所有引用共用真人／插畫骨架的正式頁自動同步。
- **工作流順序文件同步**（2026-08-03）：共同頁面依「版型／構圖 → 服裝輪廓 →
  主題材質 → 服裝改造 → 身形 → 姿勢 → 自訂 → 光影 → 背景 → 鏡頭 → 比例 →
  生成」排列；旅拍、雜誌、戰鬥學院依各自核心決策順序保留特例，實際 CSS 視覺順序與
  生成函式綁定維持一致。
- **編輯視覺設計控制層**：頁面以一鍵模板、版型、人物版位、字體、文案語氣、文字自填、語言、
  主圖形、輔助標記、色彩、影像處理、印刷質感、留白、比例與其他要求組成；模板套用後會立即
  更新預覽咒語，並透過原圖鎖定核心限制只改版面與圖形。
...
- **xianxia.html：服裝改造核心三區塊詞彙擴充，作為「9-layer」提案試點**
  ...
  改造核心」（胸口/腰側/肩部），從每區塊 8~9 個選項擴充到 19~23 個（胸口清單裡
  owner 原提的「深V／超深V剪裁」比照本站一貫尺度限制未採納）。範圍刻意只限
  xianxia.html 一頁試點，未推廣到 Magazine/Fantasy/戰鬥制服學園/旅遊。四支驗證
  腳本全過（詳見開發日誌）。
- **新增第 14 個工具頁 `ancient-goddess.html`（神話古文明女神）**
  （2026-08-02）：根據 `docs/要素不被採用的.md` 三輪累積的排除紀錄
  （希臘十字纏裹裙、敦煌樂伎神女、漢服執劍女俠等反覆出現卻未落地的
  元素），做成希臘羅馬神殿＋敦煌壁畫飛天兩大系統的新工具頁：24種服裝、
  24種材質、24種背景、9種風格、16組一鍵主題模板，並新增專屬「神話
  女神專屬姿勢」5款。`assets/core-prompt.js` 新增 `ancientGoddessCore`；
  `index.html` 新卡片歸入「奇幻世界觀寫真」分類；四支驗證腳本全數
  新增支援並全過（詳見開發日誌）。
- **「分析圖片」pipeline 第三輪執行：驗證 memory 是否真的自動生效**
  （2026-08-01）：owner 用一模一樣的原始指令句子（跟第二輪定型 memory
  時完全相同的文字）第三次要求分析圖片，這次資料夾沒有整批換掉，是在
  原本 34 張後面新增 13 張（`23.21.18`～`23.23.40`）。**這是對上一輪
  存入 memory 的 pipeline 的第一次實戰驗證**：沒有重新詢問範圍或流程，
  直接判斷只需分析新增的 13 張（已處理過的 34 張不重複做），派 2 個
  agent 平行處理，一樣要求每張圖完整五項要素分解。結果：4 張因裸露
  尺度排除（3/4 集中在中式古風類型，顯示這批供圖來源在這個主題上尺度
  偏高）、9 張安全裡逐一比對現有選項池後只有 5 個真正新增，落地到 3
  個頁面：flower-fairy.html（+1材質：水晶花冠髮飾）、floral-sweet.html
  （+1服裝+1背景：拼接碎花奔跑洋裝、歐式宮殿玫瑰石階）、kpop-idol.html
  （+2服裝：重機皮革挖空上衣、霓虹街拍綁帶上衣）。其餘 6 張安全但重複
  的（例如「執劍女俠」≈既有 `swordCultivatorAttire`+`immortalSword`
  組合、「敦煌樂伎神女」≈既有 `courtDancerRobe` 飛天舞姬視覺原型）
  跟 4 張不安全的圖，一樣完整記錄進 `docs/要素不被採用的.md`（新增
  「第三輪」章節）。四支驗證腳本全數重跑通過。
- **「分析圖片」流程正式定型為固定 pipeline，並執行第二輪 34 張新圖**
  （2026-08-01）：owner 把 `風格範例` 資料夾整批換成 34 張新圖，要求
  重跑一次同樣的分析流程，並提出兩個新規則：(1) 之後只要說「分析圖片」
  就照這套固定流程處理，不用每次重新說明；(2) 所有「有分析過但沒採用」
  的圖片要素（不管是因為裸露尺度不安全、還是因為跟現有選項重複）都要
  存進一份 `docs/要素不被採用的.md`，逐張記錄構圖要素，永久保留供未來
  研究——owner 原話：「表示這圖片我喜歡，分析完就算不採用也先別丟棄」。
  已把這套 pipeline 存成 memory（`workflow_image_analysis_pipeline.md`），
  之後任何一輪的「分析圖片」請求都會自動套用，不需要重新確認範圍。
  **這一輪的執行**：3 個 agent 平行分析 34 張圖，這次額外要求「不論是否
  採用，每張圖都要寫出完整的服裝／材質道具／背景／姿勢／光線五項要素
  分解」，才能同時滿足「新增到網站」與「存檔研究」兩個目的。34 張中 13
  張因裸露尺度不安全（其中 6 張是同一套「希臘十字露背纏裹裙」在不同
  場景/光線下的變化版本），21 張安全；21 張裡逐一比對現有選項池後，只有
  12 個是真正跟既有內容不重疊的新元素，落地到 6 個頁面：xianxia.html
  （+2背景+1服裝+1材質：落花浴池、大漠遺跡、水墨暈染麻布仙裝、石蒜
  彼岸花道具）、gala-socialite.html（+1服裝：粉鑽公主抹胸禮服）、
  isekai-fantasy.html（+1背景：夜間神社鳥居參道，狐娘巫女的服裝概念
  因為跟既有「狐娘．月下靈狐」陣營重複而不採用，只留背景）、
  floral-sweet.html（+1服裝+2背景：玫瑰胸花澎裙、地中海庭院拱廊、
  碎花壁紙鏡前自拍）、travel.html（+1服裝方向：波希米亞市集綁帶上衣）、
  fantasy-fashion.html（+1材質：報紙拼貼材質）、magazine.html（+1
  背景：彩窗光斑棚拍）。其餘安全但未採用的 11 張（多數是跟既有選項
  重複，例如「江南水鄉」≈既有 `riversideWillowPath`、「威尼斯貢多拉」
  ≈既有地點 chip「威尼斯運河」、「玫瑰花園月夜」≈既有
  `moonlitGardenTerrace`）連同全部 13 張不安全的圖，都完整記錄進
  `docs/要素不被採用的.md`（含上一輪 56 張的舊記錄）。四支驗證腳本
  全數重跑通過（`check-static`/`audit-100x` 1300 次模擬/
  `validate-preset-refs`/`build-prompt-preview`，全部 0 issue）。
  **資料品質提醒**：第一輪 56 張的原始檔案在這輪已經被 owner 整批換掉、
  磁碟上已不存在，所以研究檔案裡第一輪的記錄只能用當時 agent 產出的
  文字描述整理，深度不如這輪（這輪每張圖都有完整五項分解）；另外這輪
  部分同系列圖片（同一套服裝的多個場景版本）在平行批次處理時，agent
  的檔名編號偶爾會有一格的對應誤差，已在研究檔案裡誠實標註，不做過度
  精確的宣稱。
...
- **`index.html` 首頁導覽重新設計：拿掉頂部小連結，工具卡片依主題分 4 類**
  ...
  頂部小連結、統一成只有卡片區一套連結，並針對卡片區數量變多重新設計/
  分類。用 `AskUserQuestion` 確認分類方式，owner 選推薦方案：**真人時尚
  寫真**（寫真旅拍/雜誌棚拍/花漾甜美系/氣質名媛宴會/韓系氣質偶像風，5個，
  真人寫實時尚場景）／**奇幻世界觀寫真**（幻想廣告/中式仙俠/日式異世界/
  花仙子/戰鬥制服學園，5個，奇幻或異世界觀設定）／**動漫插畫風**（動漫
  人物美圖，1個，全站唯一插畫風格頁）／**特殊工具**（公仔系列/店家活動
  廣告，2個，非人像寫真類工具）。做法：`<nav>` 拿掉整個 `.nav-links` 區塊
  與對應 CSS（`.nav-link`/`.nav-link.active`/行動版橫向捲動樣式），
  `.nav-logo` 從連結改成純文字 `<span>`（比照其餘 13 個工具頁「不可點擊
  logo」的既有慣例，首頁現在也一致）；工具卡片區從單一 `.tools` 平鋪
  grid（`max-width:700px`，13 張卡片會排成很長的單/雙欄直向清單）改成
  `.tools-wrap`（`max-width:1080px`，可排 3 欄）包 4 個 `.tools-category`
  區塊，每區塊有一條金色小型全大寫分類標籤（`.tools-category-label`，
  右側接一條延伸到區塊邊緣的分隔線）+ 該分類的 `.tools` grid。**完全沒有
  新增/刪除/改名任何卡片或連結**，13 張卡片的 HTML 內容、`href`、配色
  class 全部原封不動搬移到對應分類區塊裡，純粹是重新分組與版面調整。
  `scripts/check-static.mjs` 確認 `index.html` 本地連結檢查通過、無重複
  id；額外用 script 確認 13 個 `.tool-card` 與 13 個 `href="*.html"` 都還在、
  HTML `<div>` 開合標籤數量相等（54/54）。這是純首頁導覽/版面調整，不涉及
  任何咒語產生器頁面的生成邏輯，四支咒語驗證腳本（`audit-100x` 等）不適用
  於 `index.html`（它沒有 inline script，純靜態導覽頁）。
...
- **`xianxia.html` 04 服裝拿掉「法袍道服」道士向分類，換成「仙靈妖精裝束」**
  ...
  符紋道長袍/藏經閣書卷袍）字面與描述都偏向道士/文人裝束，跟 owner 要的
  「美美」方向不符；`妖狐` 其實已經有獨立的「妖靈與靈獸化裝束」分類涵蓋
  （狐靈/九尾狐后等），`俠女` 也已有「古典勁裝與遊俠」分類涵蓋，所以新分類
  聚焦補齊「仙女／妖精」這塊——換成 5 個新選項：花靈瓣裙、月靈綃衣、蝶靈
  羽紗裙、霞雲輕紗裙、精怪綴飾裙，走飄逸紗裙/花瓣/蝶翼/綴飾鈴鐺這種空靈
  精靈感，不再是道袍剪裁。同步處理引用到舊 5 個服裝值的 4 組一鍵模板（原本
  「煉丹道姑」「蓮花法印」「楓葉書卷」「符紙夜巡」都是道士向命名與描述），
  重新設計成「花靈庭園」「月靈蓮池」「楓蝶精靈」「精怪夜市」，服裝／材質／
  姿勢欄位一併換成搭配新服裝美感的既有選項（例如「蓮花法印」的結印手勢
  `handSealCast` 換成蓮花池畔坐姿 `lotusPoolSit`、「符紙夜巡」的擲符手勢
